...
This document contains all the code necessary to generate the plots for Figure 3 and the related supplementary figure (S3D-F). Plots are subsequently edited in Adobe Illustrator to produce the final figures.
...
```{r}
p1 <- ggplot(metadata_filtered[metadata_filtered$COVID == "Positive" & metadata_filtered$Day %in% c("D0","D3","D7"),], aes(x = factor(Day), y = as.numeric(HALLMARK_INTERFERON_ALPHA_RESPONSE), fill = factor(Acuity.max, levels = c("5","4","3","2","1")))) + geom_boxplot() + geom_point(position = position_jitterdodge(jitter.width = 0.1)) + theme_bw()
p1$labels$fill <- c("AcuityMax")
p2 <- ggplot(metadata_filtered[metadata_filtered$COVID == "Positive" & metadata_filtered$Day %in% c("D0","D3","D7"),], aes(x = factor(Day), y = as.numeric(HALLMARK_INTERFERON_GAMMA_RESPONSE), fill = factor(Acuity.max, levels = c("5","4","3","2","1")))) + geom_boxplot() + geom_point(position = position_jitterdodge(jitter.width = 0.1)) + theme_bw()
p2$labels$fill <- c("AcuityMax")
```
...
We also check the IFN stratification by age:

```{r}
p1 <- ggplot(metadata_filtered[metadata_filtered$COVID == "Positive" & metadata_filtered$Day %in% c("D0","D3","D7"),], aes(x = factor(Day), y = as.numeric(HALLMARK_INTERFERON_ALPHA_RESPONSE), fill = factor(Age))) + geom_boxplot() + geom_point(position = position_jitterdodge(jitter.width = 0.1)) + theme_bw()
p1$labels$fill <- c("AcuityMax")
p2 <- ggplot(metadata_filtered[metadata_filtered$COVID == "Positive" & metadata_filtered$Day %in% c("D0","D3","D7"),], aes(x = factor(Day), y = as.numeric(HALLMARK_INTERFERON_GAMMA_RESPONSE), fill = factor(Age))) + geom_boxplot() + geom_point(position = position_jitterdodge(jitter.width = 0.1)) + theme_bw()
p2$labels$fill <- c("AcuityMax")
```
