...
This document contains all the code necessary to generate the plots for Figure 2 and related supplementary figures (S7-S12). Plots are subsequently edited in Adobe Illustrator to produce the final figures.
...
We first performed unbiased NMF clustering to define bulk neutrophil states in the context of acute COVID-19 infection. To do this, we used a previously described Bayesian NMF approach which identified six clusters (Freeman et al., 2021; Kim et al., 2019; Robertson et al., 2018). Clustering was performed on all samples with CIBERSORTx estimated neutrophil fraction &gt; 50% (mature neutrophils and immature neutrophils combined). This code is not yet publicly available, so we read in the outputs.
...
![NMF LFC Heatmap](COVID_Neutrophil.marker0.expr.fold.Bayes.0.ordered.full.png)

We check the extent to which these clusters are influenced by the estimated CIBERSORTx cell type proportions.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-4-1.png)

We then examined how clinical parameters varied with neutrophil NMF cluster membership among COVID-19 samples. We do not report individual-level intubation status so those lines have been commented out.
...
We used the following chunk to determine significance. Since we cannot perform the appropriate p-value correction without using the individual-level intubation status, it is commented out.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-7-1.png)

``` r
plot_grid(p7,p8,p9,p10,p11,p12,ncol=1)
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-7-2.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-7-3.png)

``` r
plot_grid(p16,p17,p18,ncol=1)
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-7-4.png)

``` r
plot_grid(p19,p20,p21,ncol=1)
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-7-5.png)

We next explore the differences between the two immature neutrophil subtypes (NMF1 and NMF4) by differential expression and gene set enrichment analysis on Days 0, 3, and 7.

DESeq2 includes built-in filters for lowly expressed genes, but we have found that there are sometimes still some outliers that have low counts in too many samples. Therefore at the beginning of each NMF differential expression analysis, we plot a curve: on the x axis, we plot the number *x* of samples required to have at least 5 counts per gene, and on the y axis, we plot the number of genes that pass this filter. We take the inflection point of this curve as the number of samples needing at least 5 counts to keep the gene.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-9-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-10-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-11-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-13-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-15-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-17-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-21-1.png)

Next we want to compare our bulk neutrophil signatures to single-cell neutrophil data from COVID-19 patients and healthy controls to see whether our clusters are reflective of the states which are defined at a single-cell resolution. To this end we use the Schulte-Schrepping Cohort 2 fresh whole blood neutrophil single-cell data. We score each cell according to the genes which define the NMF clusters.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-23-1.png)

``` r
cowplot::plot_grid(p2,p3,p4,p5,p6,p7,ncol=3)
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-23-2.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-25-1.png)

Next we compile neutrophil state gene signatures from previously published sources to contextualize our clustering results. For the NMF gene signatures, we took the top 100 genes per cluster according to log(fold-change) values. For the ARDS gene signatures, we took all genes from the microarray data from Juss et al. which were differentially expressed by at least three-fold between ARDS blood neutrophils and healthy controls. For the blood and tumor neutrophils from lung cancer patients from Zilionis et al., we took the cluster marker selection approach as described in their methods and capped each list at 100 genes maximum. For the single-cell neutrophil clusters in COVID-19 from Schulte-Schrepping et al., we filtered the positive cluster marker genes from Schulte-Schrepping et al. Table S4 Sheet 6 and again capped the lists at 100 genes maximum with ranking based on p-value. Finally, for the single-cell neutrophil clusters in the context of sepsis, we utilized publicly available data from Reyes et al. on the Broad Single Cell Portal and performed clustering in Seurat.
...
We can visualize the clustering results (not included in supplementary figures):
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-27-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-27-2.png)

``` r
n.gmt.file <- gmtPathways(paste0(prefix,"neutrophil_state_gene_sets.gmt"))
tab <- merged_seurat@assays$RNA@counts[merged_seurat@assays$RNA@counts@Dimnames[[1]] %in% n.gmt.file$NMF1,]
tab_z <- apply(tab,1,scale)
tab_z <- t(tab_z)
nmf1_meanz <- as.numeric(colMeans(tab_z))
merged_seurat <- AddMetaData(merged_seurat, metadata = nmf1_meanz, col.name = "nmf1")
tab <- merged_seurat@assays$RNA@counts[merged_seurat@assays$RNA@counts@Dimnames[[1]] %in% n.gmt.file$NMF2,]
tab_z <- apply(tab,1,scale)
tab_z <- t(tab_z)
nmf2_meanz <- as.numeric(colMeans(tab_z))
merged_seurat <- AddMetaData(merged_seurat, metadata = nmf2_meanz, col.name = "nmf2")
tab <- merged_seurat@assays$RNA@counts[merged_seurat@assays$RNA@counts@Dimnames[[1]] %in% n.gmt.file$NMF3,]
tab_z <- apply(tab,1,scale)
tab_z <- t(tab_z)
nmf3_meanz <- as.numeric(colMeans(tab_z))
merged_seurat <- AddMetaData(merged_seurat, metadata = nmf3_meanz, col.name = "nmf3")
tab <- merged_seurat@assays$RNA@counts[merged_seurat@assays$RNA@counts@Dimnames[[1]] %in% n.gmt.file$NMF4,]
tab_z <- apply(tab,1,scale)
tab_z <- t(tab_z)
nmf4_meanz <- as.numeric(colMeans(tab_z))
merged_seurat <- AddMetaData(merged_seurat, metadata = nmf4_meanz, col.name = "nmf4")
tab <- merged_seurat@assays$RNA@counts[merged_seurat@assays$RNA@counts@Dimnames[[1]] %in% n.gmt.file$NMF5,]
tab_z <- apply(tab,1,scale)
tab_z <- t(tab_z)
nmf5_meanz <- as.numeric(colMeans(tab_z))
merged_seurat <- AddMetaData(merged_seurat, metadata = nmf5_meanz, col.name = "nmf5")
tab <- merged_seurat@assays$RNA@counts[merged_seurat@assays$RNA@counts@Dimnames[[1]] %in% n.gmt.file$NMF6,]
tab_z <- apply(tab,1,scale)
tab_z <- t(tab_z)
nmf6_meanz <- as.numeric(colMeans(tab_z))
merged_seurat <- AddMetaData(merged_seurat, metadata = nmf6_meanz, col.name = "nmf6")

FeaturePlot(merged_seurat,reduction = "umap",features=c("nmf1","nmf2","nmf3","nmf4","nmf5","nmf6"),cols=brewer.pal(n = 9, name = "Reds"),pt.size=0.5)
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-27-3.png)
...
Now we have all the gene sets we will use. We first want to get a sense of how similar these gene sets are to one another, and to this end we build a network of the overlaps between gene sets.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-30-1.png)

We can also compare the neutrophil gene sets by visualizing the correlations between the mean z score for the genes in each gene set across all samples. We convert some of the gene symbols that correspond to multiple ENSEMBL IDs to the ENSEMBL ID which appears in the TPM matrix. Some of the genes that were aligned to different genome versions do not have matches and are excluded.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-32-1.png)

To identify genes and pathways in neutrophils that were associated with COVID-19 severity, we performed differential gene expression analysis between severe and non-severe COVID-19 patients for each time point.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-35-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-38-1.png)

``` r
plot2
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-38-2.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-40-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-42-1.png)

It is also of interest to view how the states evolve over time for the patients who have serial blood draws.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-44-1.png)

``` r
p2
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-44-2.png)

The ISG state seems to decline in time, though it appears to persist at late time points in the Schulte-Schrepping data according to an early/late cutoff of 10 days after symptom onset. We reanalyze the data using a sliding value for the early inclusion cutoff.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-46-1.png)

Given the large jump in the ISG cluster (C1) on Day 13 and the relatively slow cumulative increase after that, we visualize the data using Day 13 post symptom onset as the early inclusion cutoff.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-48-1.png)

Now that we have our neutrophil gene sets and differentially expressed gene lists, we can do GSEA on the results. We begin with the neutrophil states.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-50-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-52-1.png)

We note that the ARDS neutrophil signatures from Juss et al. are consistently scoring very strongly.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-55-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-56-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-57-1.png)

We compare the two sources in terms of log(fold-change) values between severe vs. mild COVID-19 and non-COVID-19 ARDS vs. healthy controls to search for any unique features. We load in the microarray summary data from Juss et al.
...
ards$NMF_overlap <- NA
ards[ards$Symbol %in% gmt.file$NMF1,]$NMF_overlap <- "NMF1"
ards[ards$Symbol %in% gmt.file$NMF2,]$NMF_overlap <- "NMF2"
ards[ards$Symbol %in% gmt.file$NMF3,]$NMF_overlap <- "NMF3"
ards[ards$Symbol %in% gmt.file$NMF4,]$NMF_overlap <- "NMF4"
ards[ards$Symbol %in% gmt.file$NMF5,]$NMF_overlap <- "NMF5"
ards[ards$Symbol %in% gmt.file$NMF6,]$NMF_overlap <- "NMF6"
ards$NMF_overlap[is.na(ards$NMF_overlap)] <- "none"

my.cols.nmf <- c(orange, skyblue, bluishgreen, yellow, blue, vermillion, "grey")
my.cols.nmf.6 <- c(orange, skyblue, bluishgreen, yellow, blue, vermillion, "grey")

fit <- lm(Day0 ~ log2FoldChange, data = ards)
p1_nmf_withlabel <- ggplot(ards, aes(x = log2FoldChange, y = Day0)) + geom_point(data=ards[ards$NMF_overlap=="none",],color="grey") + geom_point(data=ards[ards$NMF_overlap!="none",],aes(colour = factor(NMF_overlap))) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.nmf) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 0 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + geom_text_repel(data = subset(ards, NMF_overlap != "none" & abs(log2FoldChange)>log2(3)), aes(x = log2FoldChange, y = Day0, label = Symbol) ) + labs(color="NMF")
#ggsave("Day0_vs_Juss.NMF_overlap.withlabels.pdf",p1_nmf_withlabel)
p1_nmf_nolabel <- ggplot(ards, aes(x = log2FoldChange, y = Day0)) + geom_point(data=ards[ards$NMF_overlap=="none",],color="grey") + geom_point(data=ards[ards$NMF_overlap!="none",],aes(colour = factor(NMF_overlap))) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.nmf) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 0 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + labs(color="NMF")
#ggsave("Day0_vs_Juss.NMF_overlap.nolabels.pdf",p1_nmf_nolabel)

fit <- lm(Day3 ~ log2FoldChange, data = ards)
p2_nmf_withlabel <- ggplot(ards, aes(x = log2FoldChange, y = Day3)) + geom_point(data=ards[ards$NMF_overlap=="none",],color="grey") + geom_point(data=ards[ards$NMF_overlap!="none",],aes(colour = factor(NMF_overlap))) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.nmf) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 3 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + geom_text_repel(data = subset(ards, NMF_overlap != "none" & abs(log2FoldChange)>log2(3)), aes(x = log2FoldChange, y = Day3, label = Symbol) ) + labs(color="NMF")
#ggsave("Day3_vs_Juss.NMF_overlap.withlabels.pdf",p2_nmf_withlabel)
p2_nmf_nolabel <- ggplot(ards, aes(x = log2FoldChange, y = Day3)) + geom_point(data=ards[ards$NMF_overlap=="none",],color="grey") + geom_point(data=ards[ards$NMF_overlap!="none",],aes(colour = factor(NMF_overlap))) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.nmf) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 3 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + labs(color="NMF")
#ggsave("Day3_vs_Juss.NMF_overlap.nolabels.pdf",p2_nmf_nolabel)

fit <- lm(Day7 ~ log2FoldChange, data = ards)
p3_nmf_withlabel <- ggplot(ards, aes(x = log2FoldChange, y = Day7)) + geom_point(data=ards[ards$NMF_overlap=="none",],color="grey") + geom_point(data=ards[ards$NMF_overlap!="none",],aes(colour = factor(NMF_overlap))) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.nmf) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 7 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + geom_text_repel(data = subset(ards, NMF_overlap != "none" & abs(log2FoldChange)>log2(3)), aes(x = log2FoldChange, y = Day7, label = Symbol) ) + labs(color="NMF")
#ggsave("Day7_vs_Juss.NMF_overlap.withlabels.pdf",p3_nmf_withlabel)
p3_nmf_nolabel <- ggplot(ards, aes(x = log2FoldChange, y = Day7)) + geom_point(data=ards[ards$NMF_overlap=="none",],color="grey") + geom_point(data=ards[ards$NMF_overlap!="none",],aes(colour = factor(NMF_overlap))) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.nmf) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 7 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + labs(color="NMF")
#ggsave("Day7_vs_Juss.NMF_overlap.nolabels.pdf",p3_nmf_nolabel)

ards$geneofinterest <- ifelse(ards$Symbol %in% c("IL1R1","IL1R2","ARG1","CD177","MCEMP1","HLA-DMA", "HLA-DMB", "HLA-DRA", "HLA-DRB1", "HLA-DQA1"),1,0)
my.cols.severity.2 <- brewer.pal(3,"RdBu")[c(1,3)]
fit <- lm(Day0 ~ log2FoldChange, data = ards)
p1_withgoilabel <- ggplot(ards, aes(x = log2FoldChange, y = Day0)) + geom_point(data=ards[ards$geneofinterest==0,],color="grey") + geom_point(data=ards[ards$geneofinterest==1,],aes(colour = Day0>0)) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.severity.2) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 0 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + geom_text_repel(data = subset(ards, geneofinterest == 1), aes(x = log2FoldChange, y = Day0, label = Symbol),min.segment.length = unit(0, 'lines')) + labs(color="Day0 Log2(FC)")
#ggsave("Day0_vs_Juss.genesofinterest.pdf",p1_withgoilabel)

fit <- lm(Day3 ~ log2FoldChange, data = ards)
p2_withgoilabel <- ggplot(ards, aes(x = log2FoldChange, y = Day3)) + geom_point(data=ards[ards$geneofinterest==0,],color="grey") + geom_point(data=ards[ards$geneofinterest==1,],aes(colour = Day3>0)) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.severity.2) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 3 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + geom_text_repel(data = subset(ards, geneofinterest == 1), aes(x = log2FoldChange, y = Day3, label = Symbol),min.segment.length = unit(0, 'lines')) + labs(color="Day3 Log2(FC)")
#ggsave("Day3_vs_Juss.genesofinterest.pdf",p2_withgoilabel)

fit <- lm(Day7 ~ log2FoldChange, data = ards)
p3_withgoilabel <- ggplot(ards, aes(x = log2FoldChange, y = Day7)) + geom_point(data=ards[ards$geneofinterest==0,],color="grey") + geom_point(data=ards[ards$geneofinterest==1,],aes(colour = Day7>0)) + geom_smooth(method = "lm", color = "black") + theme_bw() + scale_colour_manual(values = my.cols.severity.2) + geom_vline(xintercept = c(log2(3),-log2(3)),lty = "dashed") + geom_hline(yintercept = 0, lty = "dashed") + scale_y_continuous(limits = c(-3,3)) + coord_fixed(ratio = 1.1) + ylab("Day 7 COVIDP, Severe vs. Non-Severe: log2(Fold Change)") + xlab("ARDS vs. Healthy Volunteer: log2(Fold Change)") + annotate(geom="text", x=-1, y=-2.5, label=paste0("R2 = ",summary(fit)$r.squared, "\np = ", summary(fit)$coefficients[2,4]), color="black") + geom_text_repel(data = subset(ards, geneofinterest == 1), aes(x = log2FoldChange, y = Day7, label = Symbol),min.segment.length = unit(0, 'lines')) + labs(color="Day7 Log2(FC)")
#ggsave("Day7_vs_Juss.genesofinterest.pdf",p3_withgoilabel)
```
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-59-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-60-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-61-1.png)

Finally we perform a differential expression analysis on the difference in log(fold-change) between the two sources of data.
...
We checked for overlap of NMF signature genes with the genes higher expressed in ARDS or healthy patients in the Juss data

``` r
o_nmf1_ardsup <- sum(gmt.file$NMF1 %in% ards[ards$log2FoldChange>0,]$Symbol)
o_nmf1_ardsdown <- sum(gmt.file$NMF1 %in% ards[ards$log2FoldChange<0,]$Symbol)
o_nmf2_ardsup <- sum(gmt.file$NMF2 %in% ards[ards$log2FoldChange>0,]$Symbol)
o_nmf2_ardsdown <- sum(gmt.file$NMF2 %in% ards[ards$log2FoldChange<0,]$Symbol)
o_nmf3_ardsup <- sum(gmt.file$NMF3 %in% ards[ards$log2FoldChange>0,]$Symbol)
o_nmf3_ardsdown <- sum(gmt.file$NMF3 %in% ards[ards$log2FoldChange<0,]$Symbol)
o_nmf4_ardsup <- sum(gmt.file$NMF4 %in% ards[ards$log2FoldChange>0,]$Symbol)
o_nmf4_ardsdown <- sum(gmt.file$NMF4 %in% ards[ards$log2FoldChange<0,]$Symbol)
o_nmf5_ardsup <- sum(gmt.file$NMF5 %in% ards[ards$log2FoldChange>0,]$Symbol)
o_nmf5_ardsdown <- sum(gmt.file$NMF5 %in% ards[ards$log2FoldChange<0,]$Symbol)
o_nmf6_ardsup <- sum(gmt.file$NMF6 %in% ards[ards$log2FoldChange>0,]$Symbol)
o_nmf6_ardsdown <- sum(gmt.file$NMF6 %in% ards[ards$log2FoldChange<0,]$Symbol)

ards_nmf_overlap_df <- data.frame(id=c("NMF1","NMF2","NMF3","NMF4","NMF5","NMF6"),
                                  nmf_n_genes=c(length(gmt.file$NMF1),length(gmt.file$NMF2),length(gmt.file$NMF3),
                                              length(gmt.file$NMF4),length(gmt.file$NMF5),length(gmt.file$NMF6)),
                                  ards_up_length=(rep(nrow(ards[ards$log2FoldChange>0,]),6)),
                                  ards_up_overlap_size=c(o_nmf1_ardsup,o_nmf2_ardsup,o_nmf3_ardsup,o_nmf4_ardsup,o_nmf5_ardsup,o_nmf6_ardsup),
                                  ards_down_length=(rep(nrow(ards[ards$log2FoldChange<0,]),6)),
                                  ards_down_overlap_size=c(o_nmf1_ardsdown,o_nmf2_ardsdown,o_nmf3_ardsdown,
                                                           o_nmf4_ardsdown,o_nmf5_ardsdown,o_nmf6_ardsdown),
                                       stringsAsFactors=F)
n_genes_logTPM_filtered <- nrow(logTPM_filtered)
ards_nmf_overlap_df$ards_up_overlap_p <- phyper(ards_nmf_overlap_df$ards_up_overlap_size-1,ards_nmf_overlap_df$ards_up_length,                                        n_genes_logTPM_filtered-ards_nmf_overlap_df$ards_up_length,ards_nmf_overlap_df$nmf_n_genes,lower.tail=FALSE)
ards_nmf_overlap_df$ards_down_overlap_p <- phyper(ards_nmf_overlap_df$ards_down_overlap_size-1,ards_nmf_overlap_df$ards_down_length,                                        n_genes_logTPM_filtered-ards_nmf_overlap_df$ards_down_length,ards_nmf_overlap_df$nmf_n_genes,lower.tail=FALSE)
ards_nmf_overlap_df$ards_up_notoverlap_size <- ards_nmf_overlap_df$nmf_n_genes-ards_nmf_overlap_df$ards_up_overlap_size
ards_nmf_overlap_df$ards_down_notoverlap_size <- ards_nmf_overlap_df$nmf_n_genes-ards_nmf_overlap_df$ards_down_overlap_size

m_ards_up_nmf_overlap_df <- reshape2::melt(ards_nmf_overlap_df[,c("id","ards_up_overlap_size","ards_up_notoverlap_size")])
m_ards_down_nmf_overlap_df <- reshape2::melt(ards_nmf_overlap_df[,c("id","ards_down_overlap_size","ards_down_notoverlap_size")])

star_mapping <- c("(0,0.0001]","(0.0001,0.001]","(0.001,0.01]","(0.01,0.05]","(0.05,1]")
names(star_mapping) <- c("****", "***", "**", "*", "ns")
ards_nmf_overlap_df$ards_up_overlap_stars <- names(star_mapping[cut(ards_nmf_overlap_df$ards_up_overlap_p,breaks = c(0, 0.0001, 0.001, 0.01, 0.05, 1))])
ards_nmf_overlap_df$ards_down_overlap_stars <- names(star_mapping[cut(ards_nmf_overlap_df$ards_down_overlap_p,breaks = c(0, 0.0001, 0.001, 0.01, 0.05, 1))])

g_ards_up_overlap <- ggplot(m_ards_up_nmf_overlap_df,aes(x=id,y=value)) + geom_bar(aes(fill=variable),position="stack",stat="identity") + theme_bw() + scale_fill_manual(labels=c("Up in Juss ARDS","Not up in Juss ARDS"),values=c("red","blue")) + labs(fill="Overlap") + xlab("Gene set") + ylab("Number of genes") + geom_text(data=ards_nmf_overlap_df,aes(x=id,y=105,label=ards_up_overlap_stars)) + theme(axis.text.x = element_text(colour="black"), axis.text.y = element_text(colour="black"))

g_ards_down_overlap <- ggplot(m_ards_down_nmf_overlap_df,aes(x=id,y=value)) + geom_bar(aes(fill=variable),position="stack",stat="identity") + theme_bw() + scale_fill_manual(labels=c("Down in Juss ARDS","Not down in Juss ARDS"),values=c("red","blue")) + labs(fill="Overlap") + xlab("Gene set") + ylab("Number of genes") + geom_text(data=ards_nmf_overlap_df,aes(x=id,y=105,label=ards_down_overlap_stars)) + theme(axis.text.x = element_text(colour="black"), axis.text.y = element_text(colour="black"))

g_ards_up_overlap
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-63-1.png)

``` r
g_ards_down_overlap
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-63-2.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-64-1.png)

Finally, we search for diverging gene expression patterns between severe and non-severe patients using a DESeq2 LRT model that includes a Day:severity interaction term.
...
We can visualize the effect of several genes by plotting the delta(fold-change) in severe vs. non-severe from Day 0 to Day 7. Then we can color the points according to their fold-change from Day 0 to Day 7 for severe patients only to get a sense of where the difference in fold-change is coming from.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-67-1.png)

In addition to the gene-level analyses, we can look at which pathways diverge between severe and non-severe patients based on GSEA.
...
We can visualize several key pathways at once by plotting the normalized enrichment score (NES) for the GSEA with the color of each point indicating the difference in pathway metagene score between Day 0 and 7 for severe patients.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-70-1.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-72-1.png)

``` r
p2
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-72-2.png)

Two of the top hits are GO\_GRANULOCYTE\_CHEMOTAXIS and HALLMARK\_TNFA\_SIGNALING\_VIA\_NFKB.
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-75-1.png)

``` r
p2
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-75-2.png)
...
![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-77-1.png)

``` r
p2
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-77-2.png)

``` r
p3
```

![](Figure2_S7-S12_files/figure-markdown_github/unnamed-chunk-77-3.png)
...
```
    ...
    ##  [1] corrplot_0.92               stringr_1.4.0              
    ##  [3] igraph_1.2.11               RCurl_1.98-1.5             
    ##  [5] scales_1.1.1                Matrix_1.4-0               
    ##  [7] SingleCellExperiment_1.16.0 Nebulosa_1.4.0             
    ##  [9] patchwork_1.1.1             ggalluvial_0.12.3          
    ## [11] SeuratObject_4.0.4          Seurat_4.1.0               
    ## [13] inflection_1.3.5            pheatmap_1.0.12            
    ## [15] ggpubr_0.4.0                fgsea_1.20.0               
    ## [17] cowplot_1.1.1               openxlsx_4.2.5             
    ...
    ## [33] ggplot2_3.3.5               knitr_1.37                 
    ## 
    ## loaded via a namespace (and not attached):
    ##   [1] utf8_1.2.2             reticulate_1.23        ks_1.13.4             
    ...
    ##  [16] withr_2.4.3            colorspace_2.0-2       highr_0.9             
    ##  [19] ROCR_1.0-11            ggsignif_0.6.3         tensor_1.5            
    ##  [22] listenv_0.8.0          labeling_0.4.2         GenomeInfoDbData_1.2.7
    ##  [25] polyclip_1.10-0        bit64_4.0.5            farver_2.1.0          
    ##  [28] parallelly_1.30.0      vctrs_0.3.8            generics_0.1.1        
    ##  [31] xfun_0.29              R6_2.5.1               ggbeeswarm_0.6.0      
    ##  [34] locfit_1.5-9.4         bitops_1.0-7           spatstat.utils_2.3-0  
    ##  [37] cachem_1.0.6           DelayedArray_0.20.0    assertthat_0.2.1      
    ##  [40] promises_1.2.0.1       beeswarm_0.4.0         gtable_0.3.0          
    ##  [43] globals_0.14.0         goftest_1.2-3          rlang_0.4.12          
    ##  [46] genefilter_1.76.0      splines_4.1.2          rstatix_0.7.0         
    ##  [49] lazyeval_0.2.2         spatstat.geom_2.3-1    broom_0.7.11          
    ##  [52] yaml_2.2.1             reshape2_1.4.4         abind_1.4-5           
    ##  [55] backports_1.4.1        httpuv_1.6.5           tools_4.1.2           
    ##  [58] ellipsis_0.3.2         spatstat.core_2.3-2    ggridges_0.5.3        
    ##  [61] Rcpp_1.0.8             zlibbioc_1.40.0        purrr_0.3.4           
    ##  [64] rpart_4.1-15           deldir_1.0-6           pbapply_1.5-0         
    ##  [67] zoo_1.8-9              cluster_2.1.2          magrittr_2.0.1        
    ##  [70] data.table_1.14.2      scattermore_0.7        lmtest_0.9-40         
    ##  [73] RANN_2.6.1             mvtnorm_1.1-3          fitdistrplus_1.1-6    
    ##  [76] mime_0.12              evaluate_0.14          xtable_1.8-4          
    ##  [79] XML_3.99-0.8           mclust_5.4.9           gridExtra_2.3         
    ##  [82] compiler_4.1.2         tibble_3.1.6           KernSmooth_2.23-20    
    ##  [85] crayon_1.4.2           htmltools_0.5.2        mgcv_1.8-38           
    ##  [88] later_1.3.0            tidyr_1.1.4            geneplotter_1.72.0    
    ##  [91] DBI_1.1.2              MASS_7.3-55            car_3.0-12            
    ##  [94] parallel_4.1.2         pkgconfig_2.0.3        plotly_4.10.0         
    ##  [97] spatstat.sparse_2.1-0  annotate_1.72.0        vipor_0.4.5           
    ## [100] XVector_0.34.0         digest_0.6.29          sctransform_0.3.3     
    ## [103] RcppAnnoy_0.0.19       pracma_2.3.8           spatstat.data_2.1-2   
    ## [106] Biostrings_2.62.0      rmarkdown_2.11         leiden_0.3.9          
    ## [109] fastmatch_1.1-3        uwot_0.1.11            shiny_1.7.1           
    ## [112] lifecycle_1.0.1        nlme_3.1-155           jsonlite_1.7.3        
    ## [115] carData_3.0-5          viridisLite_0.4.0      limma_3.50.0          
    ## [118] fansi_1.0.2            pillar_1.6.4           lattice_0.20-45       
    ## [121] ggrastr_1.0.1          KEGGREST_1.34.0        fastmap_1.1.0         
    ## [124] httr_1.4.2             survival_3.2-13        glue_1.6.0            
    ## [127] zip_2.2.0              png_0.1-7              bit_4.0.4             
    ## [130] stringi_1.7.6          blob_1.2.2             memoise_2.0.1         
    ## [133] irlba_2.3.5            future.apply_1.8.1
