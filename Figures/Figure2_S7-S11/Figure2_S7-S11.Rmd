...
This document contains all the code necessary to generate the plots for Figure 2 and related supplementary figures (S7-S11). Plots are subsequently edited in Adobe Illustrator to produce the final figures.
...
In Figure 2 we explore the associations between neutrophil states and COVID-19 disease severity. We begin by comparing the estimated CIBERSORTx cell type proportions with severity. 
...
First we want to check the extent to which these clusters are influenced by the estimated CIBERSORTx cell type proportions.
...
We can also explore NMF membership with COVID-19 disease status, though this analysis can only be done on Day 0. 
...
Now that we have our neutrophil gene sets and differentially expressed gene lists, we can do GSEA on the results. We begin with the pathways.
